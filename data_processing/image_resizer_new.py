...
def multiple_image_RGB_resize(folder_path):
    counter = 0
    for images_name in os.listdir("cut_data/" + folder_path):
        if (images_name.endswith(".png")):
            try:
                png_image = Image.open ("cut_data/" + folder_path + "/" + images_name)
            except:
                continue
            ...
            formatted_image.save("resized_data/" + folder_path + "_fixed/" + str(counter) + '.png', 'png')
            counter += 1
            # print(counter)
        else:
...
multiple_image_RGB_resize("Tincaeus")
print("Tincaeus done")
